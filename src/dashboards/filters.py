...
class EventParamsDashboard(BaseModel):
    ...
    def check_params(self):
        ...
        if self.status and self.status.lower() not in ["criado", "confirmado", "cancelado", "desinscrito"]:
        ...
        if self.order_by == "status":
            self.order_by = (
                f"subscription__subscription_statuses__status"
                if self.order == "asc"
                else f"-subscription__subscription_statuses__status"
            )
        else:
            self.order_by = f"{self.order_by}" if self.order == "asc" else f"-{self.order_by}"
        ...
        if self.search:
            ...
            if "@" in self.search:
                self.params &= Q(promoter__email__icontains=self.search)
...
class SubscriptionParams(BaseModel):
    ...
    def check_parms(self):
        ...
        if self.status is not None and self.status not in ["criado", "confirmado", "cancelado", "desinscrito"]:
            raise ValueError("Status inválido")

        if self.status:
            self.params &= Q(status=REVERSE_TRANSLATED_SUBSCRIPTION_STATUS[self.status])
